...
// Optimization of Index, if need to store more information turn off (usually no need)
...
struct Node
{
    ...
    int n, father, deep;
    // n: the number of subgraph, the id of father node,son[2]left son and right son,deep: current deepth
    vector<int> son;
    Graph G; // subgraph
    vector<int> color; // which subgraph the node belongs to
    Matrix dist, order;
    // border distance matrix ,border pass route with floyd k,order=(-1:connected directly)|(-2:connected in
    // parents)|(-3:connected in son nodes)|(-INF:none)
...
struct G_Tree
{
        ...
        if (y != root) // do till arrive root
        {
            ...
            for (int i = 0; i < dist2.size(); i++) {
                if (dist2[i].second == start_id) {
                    // if StartID contained，means will influence
    ...
    // del  min_car_dist which node_id=start_id from, then update. no exist return false, otherwise return true
    {
        if (node[x].father == 0)
    ...
    vector<int> KNN_min_dist_car(int S, int K) // Most import one, calculate the KNN of S, return the set of result
    {
        ...
        if (Distance_Offset == false) // no offset
        {
            while (K) {
                ...
                // Now_Catch_Dist ，distance from s to up graph
...
bool arg_has_str(char ar[], string query_str) {
    // some special data NW,US from 0
...
void knn_test(double car_percent, double change_percent, int query_number, int K, int query_time) {
    ...
    for (int t = 0; t < query_time; t++) {
        ...
        for (int i = 0; i < change_num; i++) {
            // Change the position of vehicle
